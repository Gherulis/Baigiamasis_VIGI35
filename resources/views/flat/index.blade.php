    ...
    <div class="table_container filter">
        <form action="{{ route('flat.index') }}" method="GET">
            <table>
                <thead>
                    <tr>
                        <th>Butų skaičius name : {{ $flatCount }}</th>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th colspan="2"><a href="{{ route('flat.create', request()->input('filter', '1')) }}"><button
                                class="btn_medium btn_create" value="{{ request()->input('filter', '1') }}"><i
                            ...
                            <td colspan="1" style="margin:0 auto">
                                <div class="flex-container">
                                    <a href="{{ route('flat.edit', $butas) }}">
                                        ...
                                        <button data-title="{{ $butas->invitation }}" type="submit"
                                            class="btn_small btn_show invitation-button" title="Buto pakvietimas"
                                            data-bs-toggle="modal" data-bs-target="#exampleModal1"
                                            value="{{ $butas->invitation }}"
                                            data-butas-invitation="{{ $butas->invitation }}">
                                            <i class="fa-solid fa-envelope-open"></i>
                                        </button>

                                    <form action="{{ route('flat.destroy', $butas) }}" method="POST">
                                        @csrf
                                        <button data-title="{{ $butas }}" type="submit"
                                            class="btn_small btn_delete" title="Ištrinti butą" data-bs-toggle="modal"
                                            data-bs-target="#exampleModal" value="{{ $butas }}">
